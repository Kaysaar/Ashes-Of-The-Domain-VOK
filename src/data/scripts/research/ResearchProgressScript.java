...
public class ResearchProgressScript implements EveryFrameScript {
    ...
    public void advance(float amount) {
        ...
        if(researchAPI.alreadyResearchedAmount()>=10&&researchAPI.alreadyResearchedAmountCertainTier(3)>=1){
            Global.getSector().getMemory().set("$aotd_can_op_scientist",true);
        }
        sentmessage=false;
        if (newDay()) {
            ...
            if(researchAPI.firstMarketThatHaveResearchFacility()!=null&&counter!=0&&counter%30==0&&counter<=90){
