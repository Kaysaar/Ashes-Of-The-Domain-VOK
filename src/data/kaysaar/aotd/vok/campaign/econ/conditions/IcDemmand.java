...
public class IcDemmand extends BaseMarketConditionPlugin {
    ...
    public void unapplyElectronicsDemand(BaseIndustry ind) {
        ind.supply(AoTDCommodities.ELECTRONICS, 0, "");
        ind.demand(AoTDCommodities.ELECTRONICS, 0, "");
    }
    public void applyCommodityDemandToIndustry(BaseIndustry ind, int demand){
        if (demand == 0) return;

        ind.demand(AoTDCommodities.ELECTRONICS, demand);

        ind.getDemand(AoTDCommodities.ELECTRONICS).getQuantity().unmodify(getModId());
        if (ind.getSpecialItem() != null) {
            if (ind.getSpecialItem().getId().equals(Items.CORRUPTED_NANOFORGE)) {
                ind.getDemand(AoTDCommodities.ELECTRONICS).getQuantity().modifyFlat(getModId(), -1, "Corrupted Nanoforge");
            }

            if (ind.getSpecialItem().getId().equals(Items.PRISTINE_NANOFORGE)) {
                ind.getDemand(AoTDCommodities.ELECTRONICS).getQuantity().modifyFlat(getModId(), -2, "Pristine Nanoforge");
            }
        }

        Pair<String, Integer> deficit = ind.getMaxDeficit(AoTDCommodities.ELECTRONICS);
