...
public class MaglevNetwork extends BaseIndustry {
    ...
    protected void addPostDemandSection(TooltipMakerAPI tooltip, boolean hasDemand, IndustryTooltipMode mode,boolean expanded) {
        if(mode.equals(IndustryTooltipMode.ADD_INDUSTRY)){
            tooltip.addSectionHeading("Industry Synergies", Alignment.MID,5f);
            tooltip.addPara("This structure on it's own does not offer benefits, but it can strengthen other industries, if conditions are met!", Misc.getPositiveHighlightColor(),3f);
            if(!expanded){
                tooltip.addPara("Press F1 to see all currently available on this market!",Misc.getTooltipTitleAndLightHighlightColor(),5f);
            }
            else {
                tooltip.addPara("List of all possible synergies, that we know so far",Misc.getTooltipTitleAndLightHighlightColor(),3f);
                for (BaseIndustrySynergy synergyScript : IndustrySynergiesManager.getInstance().getSynergiesNotValidForMarketInUI(market)) {
                    tooltip.setBulletedListMode(null);
                    tooltip.addPara(synergyScript.getSynergyName(),Misc.getTooltipTitleAndLightHighlightColor(),5f);
                    tooltip.setBulletedListMode("   ");
                    synergyScript.printReq(tooltip,market);
                    synergyScript.printEffects(tooltip,market,1f);
                }
...
//			}
            ...
            addPostDemandSection(tooltip, hasDemand, mode,expanded);
