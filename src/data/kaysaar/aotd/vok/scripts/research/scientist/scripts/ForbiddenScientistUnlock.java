...
public class ForbiddenScientistUnlock implements ScientistScriptUnlock {
    ...
    public void run() {
        ...
        if(manager.getAmountOfResearchFacilities()>0&&manager.haveResearched(AoTDTechIds.STREAMLINED_PRODUCTION)&&Global.getSector().getPlayerMemoryWithoutUpdate().is("$aotd_ai_core_good",true)){
            Global.getSector().getPlayerMemoryWithoutUpdate().is("$aotd_ai_core_init",true);
