...
public class GatheringPointDialog extends BasePopUpDialog {
    ProductionGatheringComponent plugin;
    ArrayList<ButtonAPI>buttons = new ArrayList<>();
    MarketAPI currentGatheringPoint;
    public GatheringPointDialog(String headerTitle, ProductionGatheringComponent plugin) {
        super(headerTitle);
        this.plugin = plugin;
    ...
    public void createContentForDialog(TooltipMakerAPI tooltip, float width) {
        for (MarketAPI playerMarket : Misc.getPlayerMarkets(true)) {
            Pair<CustomPanelAPI,ButtonAPI> pair = ProductionGatheringComponent.getMarketEntitySpriteButton(width-5,75,75,playerMarket);
