...
public class CommodityInfo implements TooltipMakerAPI.TooltipCreator {
    ...
    public void createTooltip(TooltipMakerAPI tooltip, boolean expanded, Object tooltipParam) {
        if (GPManager.getCommodities().get(id).equals(GPManager.GPResourceType.COMMODITY)) {
            ...
            CargoStackAPI stack = Global.getFactory().createCargoStack(CargoAPI.CargoItemType.SPECIAL,new SpecialItemData(id,null),null);
            stack.getPlugin().createTooltip(tooltip, expanded, null, null);
            tooltip.addSectionHeading("Global Stockpile", Alignment.MID, 5f);
            int currentTotal = GPManager.getInstance().getTotalResources().get(id);
            Color[] colors = new Color[3];
            colors[0] = Color.ORANGE;
            colors[1] = Misc.getDesignTypeColor("Abyss-Tech");
            colors[2] = Color.ORANGE;
            tooltip.addPara("Currently this faction holds %s units of %s across %s", 10f, colors, "" + currentTotal, spec.getName(),"Local Storages");
            tooltip.addPara("This is a limited resource due to it's exotic nature and means of production", 5f);
            tooltip.addSectionHeading("Stockpile data from markets", Alignment.MID, 10f);
            colors = new Color[3];
            colors[0] = Misc.getTooltipTitleAndLightHighlightColor();
            colors[1] = Misc.getPositiveHighlightColor();
            colors[2] = Color.ORANGE;
            for (MarketAPI playerMarket : Misc.getPlayerMarkets(true)) {
                if(playerMarket.hasSubmarket(Submarkets.SUBMARKET_STORAGE)){
                    SubmarketAPI submarketAPI = playerMarket.getSubmarket(Submarkets.SUBMARKET_STORAGE);
                    int amount = (int) submarketAPI.getCargo().getQuantity(CargoAPI.CargoItemType.SPECIAL,new SpecialItemData(id,null));
                    if(amount>0){
                        tooltip.addPara("%s having %s stockpiled in %s", 5f, colors, playerMarket.getName(), "" +amount, "Local Storages");
