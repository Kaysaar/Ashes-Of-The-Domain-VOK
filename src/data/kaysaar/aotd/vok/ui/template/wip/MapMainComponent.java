...
public class MapMainComponent implements ExtendedUIPanelPlugin {
    ...
    public MapMainComponent(float width, float height) {
        ...
        component = new MapZoomableComponent(
        ...
        for (SectorEntityToken jumpPoint : corvus.getJumpPoints()) {
            ...
            component.addComponent(renderer,
            ...
            if (token instanceof PlanetAPI planet) {
                ...
                tooltip.addTooltipTo(new TooltipMakerAPI.TooltipCreator() {
                    ...
                    public void createTooltip(TooltipMakerAPI tooltip, boolean expanded, Object tooltipParam) {
                        tooltip.addPara(
                                planet.getName() + ", " + planet.getTypeNameWithWorldLowerCase(),
                ...
                component.addComponent(components,
                        newLocation.x - (planet.getRadius()),
                        newLocation.y - (planet.getRadius()));
        ...
        for (CustomCampaignEntityAPI token : corvus.getCustomEntities()) {
            ...
            if (token.getCustomEntitySpec().getId().equals(Entities.STABLE_LOCATION)) {
                StableLocationComponent componentLoc = new StableLocationComponent(token);
                componentLoc.scale = component.currScale;
                Vector2f newLoc = translateCoordinatesToUI(token.getLocation());
                component.addComponent(componentLoc,
                        newLoc.x - token.getRadius() * 2,
                        newLoc.y - token.getRadius() * 2);
                continue;
            }

            if (token.getFleetForVisual() != null && token.hasTag(Tags.STATION)) {
                CampaignFleetAPI fleet = token.getFleetForVisual();
                FleetMemberAPI member = fleet.getFleetData().getMembersListCopy().stream()
                        .filter(FleetMemberAPI::isStation).findFirst().orElse(null);

                EntityRendererComponent component1 = new EntityRendererComponent(
                        member.getVariant(),
                        token.getRadius(),
                        token.getFleetForVisual().getFacing()
                );
                ...
                component.addComponent(component1,
                        newLocation.x - (token.getRadius()),
                        newLocation.y - (token.getRadius()));
