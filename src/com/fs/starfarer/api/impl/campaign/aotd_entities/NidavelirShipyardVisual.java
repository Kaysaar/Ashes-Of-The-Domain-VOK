...
public class NidavelirShipyardVisual extends BaseCustomEntityPlugin {
    ...
    protected Object readResolve(){
        ...
        reinitRendering(this.type,this.shadow,isDestroyed());
        return this;
    }
    @Override
...
//            graphics.getLocation().set(tiedToPlanet.getLocation());
    ...
    public void reinitRendering(String type,String shadowType,boolean isDestroyed){
        ...
        if(!isDestroyed){
            Planet renderer = new Planet(shadowType,radius,0.0f,new Vector2f());
...
//            layers.add(rendererOfShip4);
            for (int i = 0; i < 3; i++) {
                Planet rendererText = new Planet(type,radius+35-0.5f*i,0.0f,new Vector2f());
                rendererText.setAngle(angle);
                rendererText.setTilt(tilt);
    ...
    public  void trueInit(String type,String shadowType, PlanetAPI planetTied){
        ...
        this.tilt = tiedToPlanet.getSpec().getTilt();
        this.radius = tiedToPlanet.getRadius();
        reinitRendering(type,shadowType,isDestroyed());
        seconds = 1;
        isAppearing=true;
        elapsed = 0;
