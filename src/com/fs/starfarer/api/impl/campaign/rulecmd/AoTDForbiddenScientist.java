...
public class AoTDForbiddenScientist extends BaseCommandPlugin{
    ...
    public boolean execute(String ruleId, InteractionDialogAPI dialog, List<Misc.Token> params, Map<String, MemoryAPI> memoryMap) {
        ...
        if(arg.equals("markDecision")){
            if(Global.getSector().getImportantPeople().getPerson(getIDOfScientist())!=null){
        ...
        if(arg.equals("grantScientist")){
            ...
            ScientistPerson scientist = new ForbiddenKnowledgeScientist(Global.getSector().getImportantPeople().getPerson(getIDOfScientist()),manager.getFaction());
            manager.addScientist(scientist);
            if(manager.currentHeadOfCouncil==null){
                manager.currentHeadOfCouncil = scientist;
            }
            dialog.getInteractionTarget().getMarket().getCommDirectory().removePerson(Global.getSector().getImportantPeople().getPerson(getIDOfScientist()));
        }
